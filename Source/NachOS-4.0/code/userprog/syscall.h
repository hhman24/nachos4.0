...
/*
 * readnum means read the number of characters in the file 
 * and store it in the buffer pointed to by buffer
 */
int ReadNum();


/*
 * to output an integer to the console
 */
void PrintNum(int number);

/*
 * reads a character entered by the user
 */
char ReadChar();
...
/*
 *  returns a number random positive integer 
 */
int RandomNum();

/*
 * when user enter a string, it is stored in kernel space with buffer is user memory
 * so, we need to Tranfer data from kernelspace to user space
 */
void ReadString (char* buffer, int length);

/*
 * Print string from buffer to console
 * so, we need to Tranfer data from userpace to kernel space
 */
